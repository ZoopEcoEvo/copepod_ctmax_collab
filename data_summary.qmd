...
CTmax tended to decrease with both lower and upper latitudinal limits, but was not related to latitudinal or elevational ranges.
...
ggplot(elev_change_data, aes(x = range_lim, y = elevation, group = species_id, colour = species_id)) + 
  geom_point() + 
  geom_line() + 
  geom_label(data = filter(elev_change_data, (!(species_id %in% c("Ectocyclops phaleratus", "Thermocyclops crassus")) & range_lim == "low_lat")), 
             aes(label = species_id),
             hjust = 1,
             position = position_nudge(x = -0.01)) + 
  geom_label(data = filter(elev_change_data, (species_id %in% c("Ectocyclops phaleratus", "Thermocyclops crassus") & range_lim == "high_lat")), 
             aes(label = species_id),
             hjust = 0,
             position = position_nudge(x = 0.01)) + 
  labs(x = "Range Edge", 
