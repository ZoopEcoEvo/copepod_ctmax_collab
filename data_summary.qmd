...
species_cols = c(viridis(13, option = "mako",
                         begin = 0.3, end = 0.8), 
                 viridis(12, option = "inferno",
                         begin = 0.1, end = 0.8))

names(species_cols) = sort(unique(as.character(ctmax_data$species_id)))
...
ctmax_data %>% 
  filter(genus != "") %>% 
ggplot(aes(x = species_id, y = ctmax, fill = species_id)) + 
  geom_boxplot(outlier.colour = NA) + 
  geom_point(position = position_jitter(width = 0.1, height = 0),
             alpha = 0.5, 
             shape = 1) + 
...
ctmax_data %>% 
  ...
  ungroup() %>% 
  mutate(habitat = fct_relevel(habitat, "littoral", "surface", "metalimnion")) %>% 
ggplot(aes(x = habitat, y = mean_ctmax, colour = species_id, group = genus)) +
...
ggplot(species_effects, aes(x = species_id, y = emmean, colour = species_id)) + 
...
species_effects %>% ungroup() %>%  
  ...
  ggplot(aes(x = genus, y = emmean, group = species_id, colour = species_id)) + 
  geom_errorbar(aes(ymin = emmean - SE, ymax = emmean + SE),
                width = 0.4,
...
ggplot(aes(x = species_id, y = body_length_um, fill = species_id)) + 
  geom_boxplot(outlier.colour = NA) + 
  geom_point(position = position_jitter(width = 0.1, height = 0),
             alpha = 0.5) + 
  labs(x = "Species", 
